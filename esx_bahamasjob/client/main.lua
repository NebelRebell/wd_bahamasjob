        ...
        if data.current.value == 'dancer_outfit_7' then
            ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
              ...
              if skin.sex == 0 then
                ...
                SetPedComponentVariation(playerPed, 129, 0, 0, 0) --T-shirt
                SetPedComponentVariation(playerPed, 228, 4, 0, 0) --Torse
                SetPedComponentVariation(playerPed, 0, 0, 0, 0) --Decals
                SetPedComponentVariation(playerPed, 96, 0, 0, 0) --Bras
                SetPedComponentVariation(playerPed, 9, 7, 0, 0) --Pantalon
                SetPedComponentVariation(playerPed, 61, 0, 0, 0) --Chaussures
                SetPedComponentVariation(playerPed, 128, 0, 0, 0) --Collier

                ClearPedBloodDamage(playerPed)
                ResetPedVisibleDamage(playerPed)
                ClearPedLastWeaponDamage(playerPed)
                RequestAnimSet("MOVE_M@FAT@A")
                while not HasAnimSetLoaded("MOVE_M@FAT@A") do
                  Citizen.Wait(0)
                end
      ...
      if IsControlJustReleased(0,  Keys['E']) and IsJobTrue() then
        ...
        if CurrentAction == 'delete_vehicle' then
          ...
          if Config.EnableSocietyOwnedVehicles then
            ...
            if
              GetEntityModel(vehicle) == GetHashKey('bahamabus')
            then
...
RegisterNetEvent('NB:openMenuBahama')
AddEventHandler('NB:openMenuBahama', function()
    OpenSocietyActionsMenu()
end)
